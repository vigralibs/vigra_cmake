...
double fit_cams_lines_multi(const clif::Mat_<float>& proxy, cv::Point2i img_size, clif::Mat_<double> &lines, clif::Mat_<double> &extrinsics, clif::Mat_<double> &extrinsics_rel, clif::Mat_<double> &proj, bool vis = false, const clif::Mat_<float>& scales = clif::Mat_<float>());
